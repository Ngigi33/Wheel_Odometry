...
private:
    ...
    {
        float distance = ((rotation * 0.001) * M_PI * wheel_diameter);
        return distance;
    }
    void odom()
    {
        if (new_rot_A == 0 && new_rot_B == 0)
        {
            delta_rot_A = 0.0;
            delta_rot_B = 0.0;
        }
        ...
        delta_X = distance_Avg * cos(old_Orientation + (delta_Orientation / 2.0));
        delta_Y = distance_Avg * sin(old_Orientation + (delta_Orientation / 2.0));
        ...
        wheel_data.pose.orientation.z = old_Orientation;
        ...
        RCLCPP_INFO(this->get_logger(), "rotA=%.3f rotB=%.3f oren=%.3f D-oren=%.3f", delta_rot_A, delta_rot_B, old_Orientation, delta_Orientation);
    ...
    void publish_Quaternion()
    {
        ...
        q.setRPY(0, 0, wheel_data.pose.orientation.z);

        nav_msgs::msg::Odometry quatOdom;
        nav_msgs::msg::Odometry quatOdom_old;

        quatOdom.header.stamp = this->get_clock()->now();
        quatOdom.header.frame_id = "odom";
        quatOdom.child_frame_id = "base_link";
        quatOdom.pose.pose.position.x = wheel_data.pose.position.x;
        quatOdom.pose.pose.position.y = wheel_data.pose.position.y;
        ...
        quatOdom.twist.twist.angular.z = delta_Orientation / (quatOdom.header.stamp.sec - quatOdom_old.header.stamp.sec);

        quatOdom_old.header.stamp = quatOdom.header.stamp;
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<Distance_calc_Class>(); // creating a shared pointer to the node
